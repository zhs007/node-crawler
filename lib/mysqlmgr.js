...
class MysqlMgr {
    ...
    async start() {
        for (let key in this.mapCfg) {
            try {
                ...
                logger('info', 'MysqlMgr.start() conn ' + key + ':' + JSON.stringify(this.mapCfg[key]) + ' ok!');
            }
            catch (err) {
                logger('error', 'MysqlMgr.start() conn ' + key + ':' + JSON.stringify(this.mapCfg[key]) + ' err ' + JSON.stringify(err));
            }
        }
    }
    ...
    async onError(key, err) {
        logger('error', 'MysqlMgr.onError() conn ' + key + ':' + JSON.stringify(this.mapCfg[key]) + ' err ' + JSON.stringify(err));
        if (err.code == 'ETIMEDOUT' || err.code == 'ECONNRESET' || err.code == 'ECONNREFUSED' || err.code == 'PROTOCOL_CONNECTION_LOST' ||
            ...
            try {
                ...
                logger('error', 'MysqlMgr.onError() reconnect failed. ' + key + ':' + JSON.stringify(this.mapCfg[key]) + ' err ' + JSON.stringify(err));
            }
