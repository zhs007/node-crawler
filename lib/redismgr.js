    ...
    async __newRedisConn(cfg) {
        return new Promise((resolve, reject) => {
            ...
            redisconn.on("error", function (err) {
                logger.log('error', 'RedisMgr() redis err ' + JSON.stringify(cfg) + ' err ' + JSON.stringify(err));
    ...
    async start() {
        for (let key in this.mapCfg) {
            try {
                ...
                logger.log('info', 'RedisMgr.start() conn ' + key + ':' + JSON.stringify(this.mapCfg[key]) + ' ok!');
            }
            catch (err) {
                logger.log('error', 'RedisMgr.start() conn ' + key + ':' + JSON.stringify(this.mapCfg[key]) + ' err ' + JSON.stringify(err));
            }
        }
    }
};
