...
class Crawler_Request extends Crawler {
    ...
    async start() {
        await rp(this.options)
            .then(data => {
                logger('info', 'Crawler_Request ' + this.options.uri + ' - ' + moment().format('YYYY-MM-DD HH:mm:ss'));

                this.onProcess(data);
            })
            .catch(err => {
                logger('error', 'Crawler_Request error ' + err + ' - ' + moment().format('YYYY-MM-DD HH:mm:ss'));
