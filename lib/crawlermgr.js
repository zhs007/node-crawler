...
class CrawlerMgr {
    ...
    async _startCrawler(crawler) {
        ...
        logger.log('info', 'start crawler ' + crawler.options.uri + ' - ' + moment().format('YYYY-MM-DD HH:mm:ss'));
        logger.log('info', 'last crawler nums is ' + this.lstCrawler.length);
        ...
        logger.log('info', 'end crawler ' + crawler.options.uri);
    ...
    async _addCrawler_cache(options) {
        ...
        logger.log('info', '_addCrawler_cache ' + JSON.stringify(ret));
    }
