    ...
    if (valobj.type == 'string' || valobj.type == 'boolean' || valobj.type == 'number') {
        ...
        if (valobj.hasOwnProperty('subtype')) {
            if (valobj.subtype == 'null') {
                ...
                logger('debug', 'getVal_CDPObject subtype ' + JSON.stringify(valobj));
            }
        }
            ...
            if (valobj.className == 'Object') {
                ...
                logger('debug', 'getVal_CDPObject className ' + JSON.stringify(valobj));
            }
        }
        else {
            logger('debug', 'getVal_CDPObject object ' + JSON.stringify(valobj));
        }
    }
        ...
        logger('debug', 'getVal_CDPObject type ' + JSON.stringify(valobj));
    }
...
function parseXML(str) {
    ...
    parser.on('xmlDecl', (version, encoding, standalone) => {
        logger('debug', 'xmlDecl', version, encoding, standalone);
    });
    ...
    parser.on('entityDecl', (entityName, isParameterEntity, value, base, systemId, publicId, notationName) => {
        logger('debug', 'entityDecl', entityName, isParameterEntity, value, base, systemId, publicId, notationName);
    });
