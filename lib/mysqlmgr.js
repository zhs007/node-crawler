...
class MysqlMgr {
    ...
    async start() {
        for (let key in this.mapCfg) {
            try {
                ...
                console.log('MysqlMgr.start() conn ' + key + ':' + JSON.stringify(this.mapCfg[key]) + ' ok!');
            }
            catch (err) {
                console.log('MysqlMgr.start() conn ' + key + ':' + JSON.stringify(this.mapCfg[key]) + ' err ' + JSON.stringify(err));
            }
        }
    }
    ...
    async onError(key, err) {
        console.log('MysqlMgr.onError() conn ' + key + ':' + JSON.stringify(this.mapCfg[key]) + ' err ' + JSON.stringify(err));
        if (err.code == 'ETIMEDOUT' || err.code == 'ECONNRESET' || err.code == 'ECONNREFUSED' || err.code == 'PROTOCOL_CONNECTION_LOST' ||
            ...
            try {
                ...
                console.log('MysqlMgr.onError() reconnect failed. ' + key + ':' + JSON.stringify(this.mapCfg[key]) + ' err ' + JSON.stringify(err));
            }
...
exports.singleton = new MysqlMgr();