...
class CrawlerMgr {
    ...
    async _startCrawler(crawler) {
        ...
        logger('info', 'start crawler ' + crawler.options.uri + ' - ' + moment().format('YYYY-MM-DD HH:mm:ss'));
        logger('info', 'last crawler nums is ' + this.lstCrawler.length);
        ...
        logger('info', 'end crawler ' + crawler.options.uri);
    ...
    async _addCrawler_cache(options) {
        ...
        logger('info', '_addCrawler_cache ' + JSON.stringify(ret));
    }
