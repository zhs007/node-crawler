    ...
    if (valobj.type == 'string' || valobj.type == 'boolean' || valobj.type == 'number') {
        ...
        if (valobj.hasOwnProperty('subtype')) {
            if (valobj.subtype == 'null') {
                ...
                logger.log('debug', 'getVal_CDPObject subtype ' + JSON.stringify(valobj));
            }
        }
            ...
            if (valobj.className == 'Object') {
                ...
                logger.log('debug', 'getVal_CDPObject className ' + JSON.stringify(valobj));
            }
        }
        else {
            logger.log('debug', 'getVal_CDPObject object ' + JSON.stringify(valobj));
        }
    }
        ...
        logger.log('debug', 'getVal_CDPObject type ' + JSON.stringify(valobj));
    }
...
function parseXML(str) {
    ...
    parser.on('processingInstruction', (target, data) => {
        logger.log('debug', 'processingInstruction', target, data);
    });

    parser.on('comment', (s) => {
        logger.log('debug', 'comment', s);
    });

    parser.on('xmlDecl', (version, encoding, standalone) => {
        logger.log('debug', 'xmlDecl', version, encoding, standalone);
    });

    parser.on('startCdata', () => {
        logger.log('debug', 'startCdata');
    });

    parser.on('endCdata', () => {
        logger.log('debug', 'endCdata');
    });

    parser.on('entityDecl', (entityName, isParameterEntity, value, base, systemId, publicId, notationName) => {
        logger.log('debug', 'entityDecl', entityName, isParameterEntity, value, base, systemId, publicId, notationName);
    });
